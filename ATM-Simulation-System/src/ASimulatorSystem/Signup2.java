    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        try {
            if ((t1.getText().equals("")) || (t2.getText().equals(""))) {
                ...
                char pan_check[] = t1.getText().toCharArray();
                int count = 0, alpha = 0, num = 0, last = 0;

                if (pan_check.length >= 10) {
                    for (int i = 0; i < pan_check.length; i++) {
                        count++;
                        if (i == 9 && Character.isLetter(pan_check[i])) {
                            last = 1;
                        }
                    }

                    for (int i = 0; i < 5; i++) {
                        if (Character.isLetter(pan_check[i])) {
                            alpha++;
                        }
                    }

                    for (int i = 5; i < 9; i++) {
                        if (Character.isDigit(pan_check[i])) {
                            num++;
                        }
                    }
                }
                ...
                if (count != 10 || alpha != 5 || num != 4 || last != 1) {
                    JOptionPane.showMessageDialog(null, "Enter a Valid PAN Number");
                } else if (t2.getText().length() < 12) {
